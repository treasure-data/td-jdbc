        ...
        implements Constants
{
            ...
            throws SQLException
    {
        ...
        int semiPos = jdbcUrl.indexOf(';', Constants.URL_PREFIX.length());
        if (semiPos < 0) {
            semiPos = postUrlPos;
        }
        else {
        ...
        int slashPos = jdbcUrl.indexOf('/', Constants.URL_PREFIX.length());
        if(slashPos == -1) {
            slashPos = jdbcUrl.indexOf(';', Constants.URL_PREFIX.length());
        }
        ...
        try {
            endpoint = jdbcUrl.substring(Constants.URL_PREFIX.length(), slashPos);
        }
